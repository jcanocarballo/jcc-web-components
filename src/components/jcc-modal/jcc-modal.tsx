...
@Component({
  tag: 'jcc-modal',
  styleUrl: 'jcc-modal.css',
  shadow: true,
  assetsDirs: ['assets'],
})
export class JccModal {
        ...
        <div class="modal">
          ...
          <div class="footer">
            {this._buttons.map((button, index) => (
              <jcc-button onClick={index === 0 ? this.handleAction : this.handleCancel} text={button.text} appearance={index === 0 && this.appearance} />
            ))}
          </div>
        </div>
      </div>
